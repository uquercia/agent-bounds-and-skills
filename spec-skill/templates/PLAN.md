...
Wave 3 runs after Waves 1 and 2. Pauses at checkpoint, presents to user, resumes on approval.
...
TDD plans follow a test-first structure:
1. Write test skeleton with expected behavior
...
| `checkpoint:human-verify` | Visual/functional verification | Pauses for user review |
| `checkpoint:decision` | Implementation choices | Pauses for user decision |
...
**The automation-first rule:** `user_setup` contains ONLY what AI literally cannot do:
- Account creation (requires human signup)
- Secret retrieval (requires dashboard access)
- Dashboard configuration (requires human in browser)

**NOT included:** Package installs, code changes, file creation, CLI commands AI can run.
...
4. Verification checks must_haves against codebase
...
must_haves:
  ...
  key_links:
    - from: "src/features/user/api.ts"
      to: "src/features/user/model.ts"
      via: "TypeScript import"
      pattern: "import.*User.*from.*model"
---
...
<task type="auto">
  ...
  <read_first>src/features/user/model.ts</read_first>
...
must_haves:
  truths:
    - "Dashboard shows user stats and product listings"
...
- Always use XML structure for AI parsing
