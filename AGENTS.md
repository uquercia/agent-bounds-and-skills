...
- 需要教学时，不只给答案，要采用引导式讲解，先澄清问题再分步推进。
- 做前端时，默认追求明确视觉方向、非模板化、避免 AI 味过重的普通界面。
- 进行“流程梳理/链路说明/阶段方案”时，默认补充 Mermaid 图（如流程图、时序图或架构图），提升可读性与直观性；若场景不适合作图，再用文字说明原因。
...
## 提交信息格式约束
- 以后提交时，提交信息必须先写明本次变更属于“项目”还是“skill”，再简要概括修改内容。
- 推荐格式：`更新项目：<项目名> - <修改内容概括>` 或 `更新skill：<skill名> - <修改内容概括>`。
- 修改内容概括要短，但必须能看出本次改了什么，例如是“补充存档规范”“更新删除防护”“整理项目记录”。
- 如果一次变更同时涉及项目和 skill，优先拆成两个提交分别说明；如果必须合并，也要在提交信息里同时写明两类内容。


## Shell 与脚本执行版本约束
- 默认使用 PowerShell 7（`pwsh`）执行脚本与命令，当前固定版本为 `7.6.2`，避免继续使用 Windows PowerShell 5.1 导致兼容性报错。
- 命令示例优先采用：`pwsh -NoLogo -NoProfile -Command "<command>"`；仅当明确要求兼容旧环境时才回退到 `powershell.exe`。
- 涉及模块安装、脚本运行、编码处理、并发任务时，优先在 PowerShell 7 环境执行，减少命令行报错与无效重试。
- 若检测到当前会话不是 PowerShell 7，应先提示并切换到 `C:\Program Files\PowerShell\7\pwsh.exe` 后再执行高风险或长流程脚本。